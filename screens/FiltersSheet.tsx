        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 24, paddingBottom: 20 }}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.chipsWrap}>
              {SPORTS.map(s => (
                <TouchableOpacity
                  key={s.id}
                  style={[styles.chip, local.sport === s.id && styles.chipActive]}
                  onPress={() => update('sport', s.id)}
                >
            ...
            <View style={styles.chipsWrap}>
              {NIVEAUX.map(n => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    local.niveau === n.id && styles.chipActive,
                    local.niveau === n.id && n.color ? { backgroundColor: n.color, borderColor: n.color } : {}
                  ]}
          ...
          <View style={styles.section}>
            <View style={styles.sliderHeader}>
              ...
              <Text style={styles.sliderVal}>
                {local.distanceMax >= 200 ? '200+ km' : `${local.distanceMax} km`}
              </Text>
            </View>
            <View style={styles.sliderTrack}>
              {[10, 20, 30, 50, 75, 100, 150, 200].map(val => (
                <TouchableOpacity
                  key={val}
                  style={[styles.sliderStep, local.distanceMax >= val && styles.sliderStepActive]}
                  onPress={() => update('distanceMax', val)}
                />
              ))}
            </View>
          ...
          <View style={styles.section}>
            <View style={styles.sliderHeader}>
              ...
              <Text style={styles.sliderVal}>
                {local.deniveleMax >= 3000 ? '3000+ m' : `${local.deniveleMax} m`}
              </Text>
            </View>
            <View style={styles.sliderTrack}>
              {[100, 300, 500, 800, 1200, 1800, 2500, 3000].map(val => (
                <TouchableOpacity
                  key={val}
                  style={[styles.sliderStep, local.deniveleMax >= val && styles.sliderStepActive]}
                  onPress={() => update('deniveleMax', val)}
                />
              ))}
            </View>
            <View style={styles.sliderLabels}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.chipsWrap}>
              {DATES.map(d => (
                <TouchableOpacity
                  key={d.id}
                  style={[styles.chip, local.date === d.id && styles.chipActive]}
                  onPress={() => update('date', d.id)}
                >
          ...
          <View style={styles.section}>
            ...
            <View style={styles.chipsWrap}>
              {CRENEAUX.map(c => (
                <TouchableOpacity
                  key={c.id}
                  style={[styles.chip, local.creneau === c.id && styles.chipActive]}
                  onPress={() => update('creneau', c.id)}
                >
          ...
          <View style={styles.section}>
            ...
            <TouchableOpacity
              ...
              onPress={() => update('placesDisponibles', !local.placesDisponibles)}
            >
              <View>
...
const styles = StyleSheet.create({
  ...
  chip: {
    ...
    borderColor: '#DDD8FF', backgroundColor: '#F4F3FF',
  },
  ...
  applyBtn: {
    backgroundColor: '#5B52F0', borderRadius: 12,
